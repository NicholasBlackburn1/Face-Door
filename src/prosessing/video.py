...
# handles adding data to lists so i can tuppleize it 
    ...
    def datalist(self):
        ...
        while True:
            ...
            userdata = '{"name":'+str("[")+str(db.getName(db.getFaces(),i))+str("[")+','+'"status":'+str("[")+str(db.getStatus(db.getFaces(),i))+ str("[")+','+'"image":'+str("[")+str(db.getImageName(db.getFaces(),i))+str("[")+','+'"download_Url":'+str("[")+str(db.getImageUrI(db.getFaces(),i))+str("[")+'}'
    ...
    def downloadFacesAndProssesThem(self,logging,userData,filepath, i ):
        ...
        if(not os.path.exists(filepath+user_info['image']+".jpg")):
            ...
            logging.info("output file" +str(user_info['image']+"at"+ filepath))
        '''
...
# sends setup message and sets base image name to the current date mills and image storage path
        ...
        while True:
            ...
            logging.warn("downloaded"+str(index)+"out of "+ db.getAmountOfEntrys())
